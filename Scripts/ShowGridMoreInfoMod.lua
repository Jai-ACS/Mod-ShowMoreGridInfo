...
function ShowGridMoreInfoMod:OnRender(_)
	self:newShowGridInfo()
end

function ShowGridMoreInfoMod:newShowGridInfo()
	...
	if self.bHasResizeTextArea ~= true then
		...
		CS.Wnd_GameMain.Instance.UIInfo.m_n32:SetSize(150, 175)
...
function ShowGridMoreInfoMod:ShowGridInfo()
	...
	-- end
	-- self.nLastMouseKey = nCurMouseKey
	local curMousePosition = CS.UI_WorldLayer.Instance.MousePostion
	if curMousePosition == self.lastMousePosition and self.bForceUpdate ~= true then
		return
	end
	self.lastMousePosition = curMousePosition
	self.bForceUpdate = nil
	...
	local nCurMouseKey = GridMgr:Pos2Grid(curMousePosition, true)
